...
exports.generateFDPCertificate = async (data, db) => {
  ...
  try {
    ...
    console.log("FDP Template path:", templatePath);
    console.log("FDP CSS path:", cssPath);
    console.log("Generating FDP certificate:", data.certificateNo);
    ...
    const qr = await QRCode.toDataURL(
      `https://interncertificate.tansam.org/verify/${encodeURIComponent(data.certificateNo)}`,
      { width: 500 }
    );
...
const browser = await puppeteer.launch({
  headless: true,
    ...
    console.error("FDP CERT ERROR FULL:", err.stack || err);
    return null;
  }
};
